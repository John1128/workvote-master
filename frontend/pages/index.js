...
const addToSlackUrl = "https://slack.com/oauth/authorize?client_id=770951425620.869325594967&scope=commands"

const signUpWithSlack = (plan) => {
  window.location = addToSlackUrl + "&redirect_uri=" + encodeURI("https://workvote.petersimos.now.sh/oauth?selected=" + plan)
}
  ...
  return (
    <div className="pricing">
      <SelectablePriceCard
        ...
        name="plan_GGmYddwM7EEjWZ"
        price={0}
        features={["Non-Commercial Use", "25 polls a month"]}
        accentColor="#59A5FF"
        buttonText="Sign Up"
        title="Personal"
        onClick={select("plan_GGmYddwM7EEjWZ")}/>
      <SelectablePriceCard
        selected={selected}
        subscribed={subscribed}
...
const loginUrl = "https://slack.com/oauth/authorize?scope=identity.basic,identity.team&client_id=770951425620.869325594967"
  ...
  return (
    <Flex style={{backgroundColor: "#0099FF", marginBottom: 0, alignItems: 'center', justifyContent: 'space-between', padding: '0 45px 0 16px'}} direction="row" justify="left">
      <Flex direction="row" justify="left" className="workvote">
        <Text href="https://forms.gle/VGoNGnikEiqs6rD86" color="white" size={24} style={{marginTop: '15px'}}>fgfggf</Text>
      </Flex>
...
const SecondaryPanel = ({ selected, subscribed, subscription, setSubscribed, hasCard, setHasCard, onSuccess }) => {
  ...
  if (!subscribed && !selected) {
    return '';
  } else if (hasCard || selected === "plan_GGmYddwM7EEjWZ" || (subscribed === "plan_GGmYddwM7EEjWZ" && !selected)) {
    return (
      <ActiveSubscription
        hasCard={hasCard}
...
const App = ({user, initialSelection}) => {
  ...
  useEffect(() => {
    if (initialSelection === "plan_GGmYddwM7EEjWZ") {
      showCongratsModal();
    }
  }, [])
